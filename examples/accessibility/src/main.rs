//! # Accessibility Interaction Example
//!
//! Demonstrates the **hold_to_toggle** and **mash_auto_complete** accessibility
//! options. With these enabled, a hold interaction can be completed by pressing
//! once to start and once to confirm, rather than holding the key continuously.
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: Some(Window {
            title: "interaction/accessibility".into(),
    ...
    // Pane with hold_to_toggle enabled by default for this example
    install_demo_pane(&mut app, true);
...
fn setup_scene(mut commands: Commands) {
    ...
    // Target: 1.5s hold — but accessibility options convert it into a toggle
...
fn update_overlay(
    ...
    **overlay = Text::new(format!(
        "interaction/accessibility\n\
         Hold interactions are converted into toggles by config.\n\n\
         ...
         last result: {}\n\n\
         Pane: range {:.1} | hold x{:.2} | toggle {}\n\n\
         Controls:\n  E: interact (press to start, press to confirm)\n  Esc: cancel",
        log.last_prompt,
        log.progress * 100.0,
        log.completed,
        log.failed,
