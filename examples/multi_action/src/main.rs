//! # Multi-Action Interaction Example
//!
//! A single target exposes **two interaction slots** with different priorities.
//! Use **Tab** / **Q** to cycle the active slot, then press **E** to trigger
//! the selected one.
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: Some(Window {
            title: "interaction/multi_action".into(),
...
fn setup_scene(mut commands: Commands) {
    ...
    // Interactor with Tab/Q for slot cycling
    commands.spawn((
        Name::new("Interactor"),
        DemoInteractor,
        InteractorContext,
        ...
        actions!(InteractorContext[
            ...
            (Action::<NextAction>::new(), bindings![KeyCode::Tab]),
            (Action::<PrevAction>::new(), bindings![KeyCode::KeyQ]),
        ]),
    ));

    // Target with two slots: low-priority "Read" and high-priority "Hack"
    let slots = vec![
        InteractionSlot {
            priority: 0.1,
...
fn update_overlay(
    ...
    **overlay = Text::new(format!(
        "interaction/multi_action\n\
         Use Tab / Q to cycle slots, then press E.\n\n\
