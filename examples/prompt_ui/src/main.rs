//! # Prompt UI Example
//!
//! Demonstrates that the HUD is driven **entirely** from `InteractionPromptState`
//! and lifecycle messages. This is the same scene as "basic" but focuses on
//! showing how prompt state flows from the plugin to the UI layer.
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: Some(Window {
            title: "interaction/prompt_ui".into(),
...
fn setup_scene(mut commands: Commands) {
    ...
    // Target: same instant "Inspect" as basic — the focus is on prompt state
...
// ---------------------------------------------------------------------------
// Overlay — shows how InteractionPromptState drives the HUD
// ---------------------------------------------------------------------------
...
fn update_overlay(
    ...
    // InteractionPromptState is the authoritative source for what to display
    let prompt_state = focused.iter().next();
    let prompt_label = prompt_state
        .and_then(|s| s.offer.as_ref())
        .map(|o| o.prompt.action_label_key.clone())
        .unwrap_or_else(|| "none".to_owned());
    let has_offer = prompt_state.map(|s| s.offer.is_some()).unwrap_or(false);
